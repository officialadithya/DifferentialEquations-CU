    ...
    \subsection{Solving Linear Homogeneous Equations With Constant Coefficients: Part II}
        ...
        \begin{example}{\Difficulty\,\Difficulty\,\,Mixed Practice With Characteristic Polynomials 3}{mixedp3}
            ...
            \begin{align*}
                ...
                &=c_1e^{-2x}+c_2e^xe^{-\frac{1}{2}}e^{i\frac{\sqrt{3}}{2}}+c_3e^xe^{{-\frac{1}{2}}}e^{-\frac{i\sqrt{3}}{2}} \\
                &=c_1e^{-2x}+\tilde{c}_2e^xe^{i\frac{\sqrt{3}}{2}}+\tilde{c}_3e^xe^{-\frac{i\sqrt{3}}{2}} \\
                &=c_1e^{-2x}+\tilde{c}_2\cos\left(x\frac{\sqrt{3}}{2}\right)+\tilde{c}_3\sin\left(x\frac{\sqrt{3}}{2}\right)
            \end{align*}
            on \(\mathbb{R}\).
        \end{example}
...
\section{Lecture 16, February 27, 2023}

    \subsection{The Method of Undetermined Coefficients: Part I}

        Consider \(F(x,y\ldots,y^{(n)})=a_ny^{(n)}+\cdots+a_1y'+a_0y+Q(x)=0\). Recall that the general solution \(y(x)=y_c(x)+y_p(x)\) where \(y_c(x)\) is the general solution to \(F(x,y,\ldots,y^{(n)})_{\text{hom}}=0\). We wish to find \(y_p(x)\). If \(Q(x)\) is an elementary function and has finitely many linearly independent derivatives, we may use the Method of Undetermined Coefficients. We suppose that \(y_p(x)\) is formed by linear combinations of \(Q(x)\) and all linearly independent derivatives of \(Q(x)\). That is,
        \begin{equation*}
            y_p(x)=A_1Q(x)+\cdots+A_kQ^{(k-1)}(x).
        \end{equation*}
        and we need to find each \(A_i\). We provide the following table.
        \begin{center}
            \begin{tabular}{|c|c|}
                \hline
                \hspace{5em} & \hspace{15em} \\
                \textbf{Forcing Term \(Q(x)\)} & \textbf{General Form} \\
                & \\ \hline \hline
                & \\
                \(ae^{\beta x}\) & \(Ae^{\beta x}\) \\
                & \\ \hline
...
\section{Lecture 17, March 1, 2023}
